        ...
        if (!isSubConverterRequest && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
                ...
                if ((noTLS == 'true' && 协议类型 == atob(`\u0056\u006b\u0078\u0046\u0055\u0031\u004d\u003d`)) || 协议类型 == 'VMess') {
                        ...
                        notlsresponseBody = uniqueAddressesnotls.map(address => {
                                ...
                                if (协议类型 == 'VMess') {
                                        ...
                                        const 为烈士Link = `vless://${uuid}@${address}:${port}?encryption=none&security=&type=${type}&host=${host}&path=${path}#${encodeURIComponent(addressid + EndPS)}`;
                                        return 为烈士Link;
                                }
                ...
                const responseBody = uniqueAddresses.map(address => {
                        ...
                        if (协议类型 == 'VMess') {
                                ...
                                const 特洛伊Link = `trojan://${uuid}@${address}:${port}?security=tls&sni=${sni}&alpn=${encodeURIComponent(alpn)}&fp=randomized&type=${type}&host=${伪装域名}&path=${encodeURIComponent(最终路径)}&allowInsecure=1&fragment=${encodeURIComponent('1,40-60,30-50,tlshello')}#${encodeURIComponent(addressid + 节点备注)}`;
                                return 特洛伊Link;
                        } else {
                                const 为烈士Link = `vless://${uuid}@${address}:${port}?encryption=none&security=tls&sni=${sni}&alpn=${encodeURIComponent(alpn)}&fp=random&type=${type}&host=${伪装域名}&path=${encodeURIComponent(最终路径) + xhttp}&allowInsecure=1&fragment=${encodeURIComponent('1,40-60,30-50,tlshello')}#${encodeURIComponent(addressid + 节点备注)}`;
                                return 为烈士Link;
                        }
                                        ...
                                        function generateLink() {
                                                ...
                                                try {
                                                        ...
                                                        if (isVMess){
                                                                ...
                                                                subLink = \`https://\${domain}/sub?host=\${host}&uuid=\${uuid}&path=\${path}&sni=\${sni}&type=\${type}&alpn=\${encodeURIComponent(alpn)}&alterid=\${alterId}&security=\${security}\`;
                                                        } else {
